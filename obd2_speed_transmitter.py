...
class OBD2SpeedTransmitter:
	def __init__(self, com_port=None, baudrate=9600, timeout=0.5):
		if com_port is None:
			com_port = scan_ports()[0]
...
def __main__():
	# There should only be two options here...
	try:
		transmitter = OBD2SpeedTransmitter("/dev/ttyUSB0")
	except Exception:
		transmitter = OBD2SpeedTransmitter("/dev/ttyUSB1")
	while True:
